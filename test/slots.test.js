...
import {
  findNextFreeSlot,
  findNextFreeSlotInTimeZone,
  localDateKey,
  zonedDateKey,
} from "../extension/src/lib/slots.js";
...
test("timezone-aware cloud scheduling uses the configured local clock", () => {
  const now = new Date("2026-07-03T02:00:00.000Z"); // Fri 10:00 in Asia/Shanghai
  const slot = findNextFreeSlotInTimeZone([], PREFS, now, new Set(), "Asia/Shanghai");
  assert.equal(slot.start.toISOString(), "2026-07-03T06:00:00.000Z"); // 14:00 Shanghai
  assert.equal(slot.end.toISOString(), "2026-07-03T06:30:00.000Z");
});

test("timezone-aware cloud scheduling respects RFC3339 busy ranges", () => {
  const now = new Date("2026-07-03T02:00:00.000Z");
  const busy = [{ start: "2026-07-03T14:00:00+08:00", end: "2026-07-03T15:00:00+08:00" }];
  const slot = findNextFreeSlotInTimeZone(busy, PREFS, now, new Set(), "Asia/Shanghai");
  assert.equal(slot.start.toISOString(), "2026-07-03T07:00:00.000Z");
});

test("timezone-aware cloud scheduling skips a blocked local day", () => {
  const now = new Date("2026-07-03T02:00:00.000Z");
  const blocked = new Set([zonedDateKey("2026-07-03T06:00:00.000Z", "Asia/Shanghai")]);
  const slot = findNextFreeSlotInTimeZone([], PREFS, now, blocked, "Asia/Shanghai");
  assert.equal(slot.start.toISOString(), "2026-07-06T06:00:00.000Z"); // next Monday 14:00
});
